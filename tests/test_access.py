...
def test_emora_play_start_and_completion_persist_events(monkeypatch):
    ...
    document['quests'][0]['started_at'] -= play.timedelta(seconds=61)
    completed = play.complete_quest("one-quiet-minute", play.QuestCompletion(responses=['I noticed the sound of the fan.']), user)
