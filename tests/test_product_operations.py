...
def test_first_route_transformations_have_semantic_controls():
    ...
    assert 'aria-labelledby="launch-title"' in home
    assert 'id="how-it-works"' in home
    assert 'href="/register">Meet your Emora' in home
    ...
    assert "Product examples · your space starts fresh" in home
...
def test_chat_stage_uses_full_room_and_locked_routes_keep_legacy_motion():
    ...
    stage_rule = editorial_css.replace(" {", "{").split(".editorial-companion .chat-stage{", 1)[1].split("}", 1)[0]
    assert "width:100%" in "".join(stage_rule.split())
    assert "max-width:none" in "".join(stage_rule.split())
    assert "grid-template-rows:auto minmax(0,1fr) auto" in chat_css
    assert "scroll-behavior:auto" in chat_css
    assert 'id="chat-messages"' in chat_html
    assert 'id="jump-to-latest"' in chat_html
    assert "20260909-product-chat" in chat_html
    # Workspace content must remain visible without waiting for scroll reveals.
    assert 'element.dataset.emoraReveal = "pending"' not in system_js
