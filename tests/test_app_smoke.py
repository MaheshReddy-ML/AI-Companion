...
def test_authentication_states_share_the_doorway_shell_without_extra_registration_questions():
    ...
    for path in ["/login", "/register", "/forgot-password", "/verify-otp", "/reset-password"]:
        ...
        assert 'class="auth-letter"' in page
...
def test_home_explains_the_product_and_provides_working_entry_points():
    response = TestClient(app).get("/")
    assert response.status_code == 200
    assert 'aria-label="Emora home"' in response.text
    assert "An AI companion for whatever’s on your mind." in response.text
    assert "illustrative preview" in response.text
    for target in ('/register', '/login', '/trust', '#how-it-works', '#about'):
        assert f'href="{target}"' in response.text
    for name in ('talk', 'reflect', 'focus', 'research'):
        assert f'aria-controls="demo-{name}"' in response.text
        assert f'id="demo-{name}" role="tabpanel"' in response.text
...
def test_emora_brand_mark_is_available_across_platform_surfaces():
    client = TestClient(app)
    favicon = '<link rel="icon" type="image/png" sizes="64x64" href="/static/images/emora-logo-v2-64.png?v=20260911-emora-brand" />'
    ...
    for path in ["/login", "/register", "/forgot-password", "/verify-otp", "/reset-password"]:
        assert 'href="/" aria-label="Emora home"' in client.get(path).text
...
def test_play_keeps_features_inside_the_cinematic_world():
    ...
    assert 'aria-label="Play sections"' in response.text
    assert "play-product.css" in response.text
...
def test_product_depth_stays_inside_existing_sections():
    ...
    assert 'id="mobile-navigation-dialog"' in companion
    assert 'href="/play"' in companion
    ...
    assert "var(--at-bg)" in companion_css
...
def test_workspace_sections_expose_working_selection_and_empty_state_contracts():
    ...
    assert 'class="profile-category-nav" aria-label="Profile settings sections"' in profile
    assert 'data-section-panel="profile-privacy"' in profile
...
def test_companion_room_uses_compact_spacing_for_laptop_height():
    ...
    assert "companion-chat.css?v=20260909-product-chat" in page
...
def test_chat_environment_picker_is_persistent_accessible_and_overview_aligned():
    ...
    assert "--editorial-teal:#81aef7" in "".join(workspace.split())
...
def test_final_responsive_light_and_icon_regressions_are_present():
    ...
    assert 'class="chat-actions-menu"' in client.get("/chat").text
    assert 'aria-label="Message Emora"' in client.get("/chat").text
    assert "env(safe-area-inset-bottom)" in companion
    assert "overflow-y:auto" in companion
