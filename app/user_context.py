...
from app.companion_identity import companion_name, personalization_context
import re
...
class UserContextBuilder:
    ...
    def build(self) -> str:
        ...
        if personalization_context(self.user):
            lines.append(personalization_context(self.user))
        if preference_lines:
        ...
        lines.extend([
            "Use memories only when relevant to the latest request; never announce retrieval or memory counts. Prefer explicit current corrections over older facts. Do not assume the current user is a project creator. Use the authenticated profile over conflicting old chat or memory. If the name is not provided, never invent one.",
