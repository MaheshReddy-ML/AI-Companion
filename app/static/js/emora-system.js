...
// Content stays visible throughout scrolling and back navigation. Avoid delayed
// blur reveals on long pages and the extra observer/layout work they require.
...
function initAuthJourney() {
  ...
  if (!["Login", "Register"].includes(body.dataset.pageTitle)) panel.prepend(progress);
...
function initChatViews() {
  ...
  const controls = [...document.querySelectorAll("button[data-chat-view]")];
...
function initProfileControlRoom() {
  const main = document.querySelector(".profile-settings-page:not(.profile-product) main");
  if (!main) return;
  const zones = [
