...
async function searchWorkspace(query) {
  ...
  try {
    const data = await apiRequest(`/api/workspace/search?q=${encodeURIComponent(query)}`, { auth: true, cache: "no-store", signal: searchController.signal });
    ...
    status.textContent = error.message || "Search is unavailable right now."; renderSearch([]);
  }
}
...
input?.addEventListener("input", () => { searchGeneration += 1; searchController?.abort(); window.clearTimeout(searchTimer); searchTimer = window.setTimeout(() => searchWorkspace(input.value.trim()), 220); });
