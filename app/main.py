...
async def secure_browser_defaults(request: Request, call_next):
    ...
    _set_default_header(response, "Content-Security-Policy-Report-Only", CONTENT_SECURITY_POLICY.replace("script-src 'self'", "script-src 'self' 'wasm-unsafe-eval'") if request.url.path == "/your-emora" else CONTENT_SECURITY_POLICY)
...
def favicon() -> RedirectResponse:
    return RedirectResponse(url="/static/images/emora-logo-v2-64.png?v=20260911-emora-brand")
