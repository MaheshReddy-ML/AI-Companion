...
from app.play_verification import QuestCompletion, requirements, validate_completion
...
def _serialize_play_quest(quest: dict, stored: dict | None, history: dict, user: dict) -> dict:
    ...
    return {
        **quest,
        ...
        "lockReason": reason,
        "verification": requirements(quest["id"], stored),
    }
...
def start_quest(quest_id: str, current_user: dict = Depends(get_current_user)) -> dict:
    ...
    now = utc_now()
    document = collection.find_one_and_update(
        {"user_id": current_user["_id"], "date": today, "quests": {"$elemMatch": {"id": quest_id, "completed": {"$ne": True}, "started_at": {"$exists": False}}}},
    ...
    if not document:
        ...
        if existing:
            saved = next((item for item in existing.get('quests', []) if item.get('id') == quest_id), {})
            ...
            if saved.get('started_at'):
                return {"state": "IN_PROGRESS", "startedAt": to_iso(saved['started_at']), "serverNow": to_iso(now), "verification": requirements(quest_id, saved)}
    ...
    return {"state": "IN_PROGRESS", "startedAt": to_iso(now), "event": event_type, "serverNow": to_iso(now), "verification": requirements(quest_id, {"started_at": now})}


@router.post("/quests/{quest_id}/complete")
def complete_quest(quest_id: str, payload: QuestCompletion, current_user: dict = Depends(get_current_user)) -> dict:
    ...
    if not quest or not _play_accessible(quest, current_user, before_history["total"]):
        raise HTTPException(status_code=403, detail="This experience is still locked.")
    today_document = next((item for item in before_documents if item.get('date') == day_key), {})
    stored = next((item for item in today_document.get('quests', []) if item.get('id') == quest_id), None)
    if stored is None:
        raise HTTPException(404, "Today's experience was not found.")
    if stored.get('completed'):
        return {"message": "Already complete.", "completed": before_history["total"], "events": [], "state": "COMPLETED"}
    verification = validate_completion(quest_id, stored, payload, now)
    result = collection.update_one(
        {"user_id": current_user["_id"], "date": day_key, "quests": {"$elemMatch": {"id": quest_id, "completed": {"$ne": True}, "started_at": stored["started_at"]}}},
        {"$set": {"quests.$.completed": True, "quests.$.state": "COMPLETED", "quests.$.completed_at": now, "quests.$.verification": verification}},
    )
