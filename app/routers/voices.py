...
async def speak(req: SpeakRequest, request: Request, current_user: dict = Depends(require_entitlement("voice"))):
    ...
    if not manager.has_speakable_text(
        text, voice_id=assignment["voice_id"], companion_id=companion_id,
        speech=req.speech, brain=req.brain,
    ):
        raise HTTPException(status_code=422, detail="No speakable text remains after TTS sanitization.")
    headers = {
